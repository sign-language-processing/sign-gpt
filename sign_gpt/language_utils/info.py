...
IANASignedLanguages = [
    ...
    {"signed": 'rsl-by', "spoken": 'be', "country": 'by', "abbreviation": ''},
    # Russian Sign Language (in Belarus, instead of Belarusian Sign Language)
    {"signed": 'gss-cy', "spoken": 'el', "country": 'cy', "abbreviation": ''},
    # Greek Sign Language (in Cyprus, instead of Cypriot Sign Language)
]


def sign_language_by_abbreviation(abbreviation: str):
    for lang in IANASignedLanguages:
        if lang["abbreviation"].lower() == abbreviation.lower():
            return lang["signed"]

    print(f"Could not find sign language with abbreviation {abbreviation}")
