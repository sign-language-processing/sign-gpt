...
def load_datasets(mapping_fn, hide_test=True, **kwargs):
    # Load train, dev, and test splits
    train_dataset = load_split_data('train', mapping_fn, **kwargs)
    validation_dataset = load_split_data('validation', mapping_fn, **kwargs)

    test_mapping = (lambda datum: dataset_test_mapping_fn(datum, mapping_fn)) if hide_test else mapping_fn
    test_dataset = load_split_data('test', test_mapping, **kwargs)
